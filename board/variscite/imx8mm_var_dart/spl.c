...
{
	int board_id = var_get_board_id();

	if (board_id == DART_MX8M_MINI) {
		var_eeprom_read_header(&eeprom);
		var_eeprom_adjust_dram(&eeprom, &dram_timing_lpddr4);
		ddr_init(&dram_timing_lpddr4);
	}
	else if (board_id == VAR_SOM_MX8M_MINI) {
		var_eeprom_read_header(&eeprom);
...
int board_mmc_init(bd_t *bis)
{
	...
	for (i = 0; i < CONFIG_SYS_FSL_USDHC_NUM; i++) {
		...
		case 0:
			...
			if (var_get_board_id() == VAR_SOM_MX8M_MINI) {
...
int board_fit_config_name_match(const char *name)
{
	...
	if ((board_id == DART_MX8M_MINI) && !strcmp(name, "imx8mm-var-dart-customboard"))
		return 0;
	else if ((board_id == VAR_SOM_MX8M_MINI) && !strcmp(name, "imx8mm-var-som-symphony"))
		return 0;
	else
		return -1;
}
