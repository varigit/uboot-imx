...
struct efi_fw_image fw_images[] = {
	{
		.image_type_id = NAVQP_BOOT_IMAGE_GUID,
		.fw_name = u"NAVQP-RAW",
		.image_index = 1,
	},
};
...
void ss_mux_select_port1(enum typec_cc_polarity pol)
{
...
static int setup_typec(void)
{
	...
	ret = tcpc_init(&port2, port2_config, &ss_mux_select_port2);
	if (ret) {
	...
	ret = tcpc_init(&port1, port1_config, &ss_mux_select_port1);
	if (ret) {
...
int board_usb_init(int index, enum usb_init_type init)
{
	...
	port = (index == 0 ? &port1 : &port2);
	dwc3_device_data = (index == 0 ? &dwc3_device_data1 : &dwc3_device_data2);
